...
class IQL:
    ...
    def act(self, obss) -> List[int]:
        """Epsilon-greedy action selection for the stateless task
    ...
    def learn(
        ...
        updated_values = []
        for i in range(self.num_agents):
            obs, action, reward, n_obs = obss[i], actions[i], rewards[i], n_obss[i]

            if done:
                target = reward
            else:
                next_q = max(self.q_tables[i][str((n_obs, a))] for a in range(self.n_acts[i]))
                target = reward + self.gamma * next_q

            key = str((obs, action))
            self.q_tables[i][key] += self.learning_rate * (target - self.q_tables[i][key])
            updated_values.append(self.q_tables[i][key])
        return updated_values

    def update(self, *args, **kwargs):
        """Alias for learn(), matching the "update" naming used in the assignment text.
        train_iql.py calls agents.learn(...) directly, so learn() remains the real
        implementation; this just forwards to it under the alternative name.
        """
